...
DATA_SET_NAME = 'unbiased_analysis_NIH'
BATCH_TO_RUN = 'batch3'

INPUT_DIR = os.path.join(BASE_DIR,'cell_profiler', 'outputs','cell_profiler_RUNS', DATA_SET_NAME)
INPUT_DIR_BATCH = os.path.join(INPUT_DIR, BATCH_TO_RUN)
OUTPUT_DIR = os.path.join(BASE_DIR,'cell_profiler', 'outputs', 'combined', DATA_SET_NAME, BATCH_TO_RUN)
...
marker_dict = {'G3BP1':[], 'KIF5A':[], 'PURA':[], 'P54':[], 'TUJ1':[], 'DAPI':[],
            'TDP43':[], 'FMRP':[], 'SQSTM1':[], 'Phalloidin':[], 'PSD95':[], 'CLTC':[],
            'NEMO':[], 'DCP1A':[], 'GM130':[], 'TOMM20':[], 'FUS':[], 'NCL':[], 'SNCA':[], 'ANAX11':[],
...
def retrieve_features(input_path):
    ...
    for file in os.listdir(input_path):  
        ...
        if 'Object' not in str(object_type): 
            continue                      #process only files with object or nucleus measurements
    ...
    with os.scandir(batch_path) as input_data_folder:
        for entry in input_data_folder:
            ...
            # if that's a marker directory
                ...
                #skip stress
                condition_dir = pathlib.Path(entry.path).parent.parent.name
                if condition_dir.casefold() == 'stress':
                    continue
...
def combine_markers(files_path):
    ...
    with os.scandir(files_path) as files_folder:
        for entry in files_folder:
            # only process CSV files; skip folders like 'plots'
...
#    nucleus_markers = ['FMRP', 'CLTC', 'DCP1A', 'PEX14', 'PURA', 'G3BP1', 'ANAX11', 'Calreticulin', 'GM130', 'CD41', 'FUS']
