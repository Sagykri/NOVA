...
INPUT_DIR = os.path.join(BASE_DIR, 'cell_profiler', 'outputs', 'combined', 'unbiased_analysis_NIH')
INPUT_DIR_BATCH = os.path.join(INPUT_DIR, BATCH_TO_RUN)
OUTPUT_DIR = os.path.join(INPUT_DIR, BATCH_TO_RUN, 'plots')

LOG_DIR_PATH = os.path.join(BASE_DIR, 'cell_profiler','outputs', 'logs')
     ...
     with os.scandir(input_path) as input_data_folder:
        ...
        for features_file in input_data_folder:
            ...
            for name, group in groups:
                ...
                A.columns = [col+'_PrimaryObject1' if col not in ['object_type', 'replicate', 'treatment', 'cell_line', 'Parent_Nucleus', 'marker', 'panel'] else col for col in A.columns]
                B.columns = [col+'_PrimaryObject2' if col not in ['object_type', 'replicate', 'treatment', 'cell_line', 'Parent_Nucleus', 'marker', 'panel'] else col for col in B.columns]
                C.columns = [col+'_SecondaryObject' if col not in ['object_type', 'replicate', 'treatment', 'cell_line', 'Parent_Nucleus', 'marker', 'panel'] else col for col in C.columns]
        ...
        if stress:
            df_all.to_csv(os.path.join(INPUT_DIR_BATCH, f'stress_all_markers_concatenated-by-object-type_{BATCH_TO_RUN}.csv'), index=False)
        else:
...
def preprocessing(features_df, use_condition='Untreated', exclude_cols=['Parent_Nucleus', 'object_type', 'Unnamed: 0_PrimaryObject1', 'Unnamed: 0_PrimaryObject2', 'Unnamed: 0_SecondaryObject']):
    ...
    cols_to_convert = [col for col in df.columns if col not in ['replicate', 'treatment', 'cell_line', 'marker', 'panel']]
    ...
    # Replace inf values with the maximum finite value of that column
    def replace_inf_with_finite_max(df):
            for col in df.select_dtypes(include=[np.number]).columns:
                inf_mask = np.isinf(df[col])
                if inf_mask.any():
                    print('replacing inf values in', col)
                    finite_max = df.loc[np.isfinite(df[col]), col].max()
                    df.loc[inf_mask, col] = finite_max
    replace_inf_with_finite_max(df)
    return df
...
def plot_umap1(df_all, color_mappings=None):
    ...
    umap_df = df_all.drop(['replicate', 'panel'], axis=1, inplace=False)
    ...
    for idx, mapper in zip([indices1, indices2, indices3], [embedding1, embedding2, embedding3]):
        ...
        colors = idx.map(color_map)
        scatter = ax.scatter(mapper[:, 0], mapper[:, 1], s=5, c=colors, alpha=0.6, cmap=cmap)

        # Legend
        handles = [
            mpatches.Patch(
                color=color_map[label] if cmap is None else scatter.cmap(scatter.norm(color_map[label])),
