...
class BaseTrainer:
    ...
    def __init__(
        ...
        # Sagy
        self.count_lr_no_improve = None
        self.count_early_stop = None
        ##
        ...
        if self.optimizer is None:
            ...
            if count_lr_no_improve >= self.train_args['reducelr_patience']:
                if self.optimizer.param_groups[0]['lr'] > self.train_args['min_lr']:
                    ...
                    logging.info(f"learn rate = {self.optimizer.param_groups[0]['lr']}")
                    return 0
                else:
        ...
        torch.save(
            {
                ...
                'count_lr_no_improve': self.count_lr_no_improve, # SAGY
                'count_early_stop': self.count_early_stop #SAGY
            },
            fpath,
        )
    ...
    def save_model(
        ...
        logging.info(f'A model has been saved at {fpath}')
    ...
    def load_model(self, path: str, by_weights: bool = True):
        ...
        if isinstance(_model, dict):
            ...
            if by_weights:
                ...
                self.model = _model
        logging.info(f'A model has been loaded from {path}')
