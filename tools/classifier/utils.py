...
def plot_confusion_matrix(y_true, y_pred, label_encoder, shorten_labels=True, rotation=90):
    ...
    disp.plot(xticks_rotation=rotation, ax=ax, cmap='viridis')  # optional: use consistent colormap
    plt.tight_layout(rect=[0, 0.05, 1, 1])  # give extra space at the bottom
    plt.show()
    
def load_batches_pkl(batch_ids, umap=1):
    """Load and concatenate embeddings and labels from a list of batch numbers."""
    X_list, y_list = [], []
    for b in batch_ids:
...
def load_batches(
    ...
    y = np.concatenate(y_list, axis=0).reshape(-1)  # 1D labels
    return X, y
...
def run_baseline_model(
    ...
    if test_specific_batches:
        iter_tests = [[b] for b in test_specific_batches]   # each test batch separately
    else:
    ...
    for test_batches in iter_tests:
        ...
        # Always encode labels numerically
        le = LabelEncoder()
...
def run_cluster_eval(batches, pca_options, norm_options, balance_options, cluster_range, method = 'kmeans'):
    results = []
    ...
    for pca_flag, norm_flag, balance_flag, n_clusters in product(pca_options, norm_options, balance_options, cluster_range):
        ...
        df, _, _ = run_clustering(batches=batches, method=method, cluster_kwargs=cluster_kwargs,
...
def compute_multilabel_metrics(conf_matrices, labels=None):
    """
    Computes per-class and macro-averaged accuracy, sensitivity, specificity, PPV, and NPV
    ...
    macro = df[['Accuracy', 'Sensitivity', 'Specificity', 'PPV', 'NPV']].mean(skipna=True)
