...
# from config_MG132_ML240_Etoposide import CONFIG, FILENAME_POSTFIX, FOLDERS, EXCLUDE_SUB_FOLDERS, DST_ROOT_PATH, FILE_EXTENSION, KEY_CELL_LINES, KEY_MARKERS, KEY_MARKERS_ALIAS_ORDERED, KEY_REPS, LOGGING_PATH, SRC_ROOT_PATH, KEY_COL_WELLS, KEY_ROW_WELLS
...
class Utils():
    ...
    def get_folders_to_handle(self):
        def get_subfolders(root):
            return [os.path.join(root, sub) for sub in os.listdir(os.path.join(self.config.SRC_ROOT_PATH, root)) if os.path.join(root, sub) not in self.config.EXCLUDE_SUB_FOLDERS]
        
        def get_processed_subfolders(folders):
            folders = []
            for f in self.config.FOLDERS:
                f_depth = get_subfolders(f)
                folders.extend(f_depth)
            return folders
    ...
    def init_folders(self, folder):
        ...
        datestamp, batch, panel = self.__get_folder_info(folder)
    ...
    def copy_files(self, folder_path, panel, batch, cut_files=False):
        ...
        for f in files_names:
            ...
            marker_alias, indx = self.__get_file_info(file_name)
