...
def show_processed_tile(df, index=0):
    ...
    ax[0].set_title(f'{image_name}/{tile} - Target: {round(get_image_focus_quality(site_image[:, :, 0]), 2)}, {round(compute_entropy(site_image[:, :, 0]), 2)}, {round(compute_snr(site_image[:, :, 0]), 2)}', fontsize=11)
    ax[0].imshow(site_image[:, :, 0], cmap='gray', vmin=0, vmax=1)
    ax[0].set_axis_off()

    ax[1].set_title(f'{image_name}/{tile} - Nucleus: {round(get_image_focus_quality(site_image[:, :, 1]), 2)}, {round(compute_entropy(site_image[:, :, 1]), 2)}, {round(compute_snr(site_image[:, :, 1]), 2)}', fontsize=11)
...
def extract_umap_data(
    ...
    folder_pattern = re.compile(
        ...
        r"(?P<markers>[A-Za-z0-9-_,]+|all_markers|without_[A-Za-z0-9-]+)_colored_by_(?P<coloring>.+)"
    )
        ...
        if os.path.exists(folder_path):
            ...
            for root, _, files in os.walk(folder_path):
                ...
                if match:
                    ...
                    # Extract markers, including handling 'without_markerX'
...
def get_pickle_path(df_umaps, batch, umap_type, reps, coloring, marker, cell_line, base_dir="/"):
    """
