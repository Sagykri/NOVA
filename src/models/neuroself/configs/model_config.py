...
class TLNeuroselfB78NoDS_for_fc_ModelConfig(NeuroselfConfig):
    def __init__(self):
        super().__init__()
...
class TLNeuroselfB78NoDSNoDecoderModelConfig(NeuroselfConfig):
    def __init__(self):
        super().__init__()
        
        self.OUTPUTS_FOLDER = MODEL_OUTPUT 
        self.MODEL_OUTPUT_FOLDER = os.path.join(self.OUTPUTS_FOLDER, 'models_outputs_batch78_nods_tl_ep23_NO_decoder') #SAGY030624
        
        self.LOGS_FOLDER = os.path.join(self.MODEL_OUTPUT_FOLDER, 'logs')
        self.CONFIGS_USED_FOLDER = os.path.join(self.MODEL_OUTPUT_FOLDER, "configs_used", datetime.datetime.now().strftime("%d%m%y_%H%M%S_%f"))

        # Models
        self.PRETRAINED_MODEL_PATH = None #SAGY030624 #os.path.join(self.OUTPUTS_FOLDER,"models_outputs_cytoself_qsplit9", "checkpoints", "checkpoint_ep23.chkp") 
        self.MODEL_PATH = "/home/labs/hornsteinlab/Collaboration/MOmaps_Sagy/MOmaps/outputs/models_outputs_batch78_nods_tl_ep23_NO_decoder/model_17.pt"  #None# #SAGY030624 os.path.join(self.MODEL_OUTPUT_FOLDER, 'model_31.pt')
        self.LAST_CHECKPOINT_PATH = os.path.join(self.MODEL_OUTPUT_FOLDER, 'checkpoints')
...
class TLNeuroselfdeltaNLSB25ModelConfig(NeuroselfConfig):
    def __init__(self):
        ...
        self.MODEL_PATH = os.path.join(self.OUTPUTS_FOLDER, "models_outputs_deltaNLS_tl_neuroself_sep_TDP43", "model_22.pt")  
        self.LAST_CHECKPOINT_PATH = os.path.join(self.MODEL_OUTPUT_FOLDER, 'checkpoints')
...
class TLNeuroselfNiemannPickB14ModelConfig(NeuroselfConfig):
    def __init__(self):
        super().__init__()
