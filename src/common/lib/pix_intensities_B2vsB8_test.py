...
def test_raw_pixels(batch_num):
    ...
    if batch_num==2:
        ...
        values = []
        stats = []
        site_image = cv2.imread(input_data_dir, cv2.IMREAD_GRAYSCALE)
        ...
        for site_image_file_name in site_images:
            site_image = cv2.imread(os.path.join(input_data_dir, site_image_file_name, cv2.IMREAD_GRAYSCALE)
...
def test_fiji_pix_scale():
     ...
     for file in os.listdir(os.path.join(DATA_DIR, 'raw')):         
         pre_fiji_img = cv2.imread(os.path.join(DATA_DIR, 'raw', file), cv2.IMREAD_GRAYSCALE)
         post_fiji_img = cv2.imread(os.path.join(DATA_DIR, 'converted', file), cv2.IMREAD_GRAYSCALE)
