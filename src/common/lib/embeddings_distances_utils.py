...
def fetch_saved_embeddings(config_model, config_data, embeddings_type, EMBEDDINGS_NAMES, exclude_DAPI=True):
    ...
    logging.info(f"[load_embeddings] before reshape: {all_embedings_data.shape}, {all_labels.shape}")   
    all_embedings_data = np.transpose(all_embedings_data, (0, 2, 3, 1)).reshape(all_embedings_data.shape[0], -1)    
    #all_embedings_data = all_embedings_data.reshape(all_embedings_data.shape[0], -1)
    logging.info(f"[load_embeddings] after reshape: {all_embedings_data.shape}, {all_labels.shape}")    
