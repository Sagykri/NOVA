...
def train_vit_contrastive(config_path, config_data_path):
    ...
    dataset = Dataset.get_subset(dataset_train, train_indexes)
    dataset_val = Dataset.get_subset(dataset_train, val_indexes)
    ...
    logging.info(f"num_classes = {config.num_classes}; pretrained model num_classes = {config.pretrained_model_num_classes}")
    logging.info(f"pretrained model path = {config.pretrained_model_path }")
    logging.info(f"include head? = {config.include_head}")
    logging.info('positive and negative are from random rep and batch (but not same index, meaning not the same *tile*)')
    ...
    model = create_vit(
                ...
                num_classes=config.pretrained_model_num_classes 
        ).cuda()

    pretrained_model_path = config.pretrained_model_path 
    logging.info(f"Loading pretrained model ({pretrained_model_path})")
    model = utils.load_model_from_checkpoint(pretrained_model_path, model)
    
    if not config.include_head:
        logging.info(f"include_head = {config.include_head}. Dropping the head (changing it to identity())")
        model.head = nn.Identity()
    else:
        logging.info(f"Changing the head output dim from {model.head.out_features} to {config.num_classes}  (include_head={config.include_head})")
        model.head = nn.Linear(model.head.in_features, config.num_classes)
    logging.info(f"Updated head: {model.head}")

    model = model.cuda()
    
    ####################
    # Freeze layers 
    if hasattr(config, 'layers_to_freeze') and len(config.layers_to_freeze) > 0:
        logging.info(f"Freezing layers: {config.layers_to_freeze}")
        _freezed_layers = fine_tuning_utils.freeze_layers(model, config.layers_to_freeze)
