...
class VectorQuantizer(nn.Module):
    ...
    def _calc_metrics(
        self, z: Tensor, z_quantized: Tensor, encoding_onehot: Tensor
    ...
    def forward(self, z: Tensor):
        ...
        #     z_quantized.view((z.shape[0],) + z.shape[1:] + (self.embedding_dim,)), -1, 1
        # ).contiguous()

        # compute metrics
        loss, perplexity = self._calc_metrics(
            z, z_quantized, encoding_onehot
        )
        # reshape back to match original input shape
        ...
        return (
            ...
            # encoding_indices.view((-1, 1) + z.shape[2:]),
