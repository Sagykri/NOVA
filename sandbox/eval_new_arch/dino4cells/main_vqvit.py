...
def train_vqvit(config, config_data_path):
    ...
    vit_loss = VQVITLoss(fc_coeff=config.fc_coeff,
                         vq_coeff=config.vq_coeff,
                         label_smoothing=config.label_smoothing).cuda()#nn.CrossEntropyLoss(label_smoothing=0.2).cuda()
...
def forward_pass(model, res, vit_loss):
    ...
    with torch.cuda.amp.autocast():
        ...
        outputs, vq_loss, perplexity = model(images)
        loss, fc_loss, vq_loss = vit_loss(outputs, targets, vq_loss)
    
    return loss, fc_loss, vq_loss, perplexity
...
def train_one_epoch(
    ...
    for it, res in enumerate(data_loader):
        ...
        loss, fc_loss, vq_loss, perplexity = forward_pass(model, res, vit_loss)
        loss = loss / args.accumulation_steps  # Normalize the loss
        running_loss += loss.item()
        
        fc_loss = fc_loss / args.accumulation_steps
        vq_loss = vq_loss / args.accumulation_steps
        ...
        if (it + 1) % args.accumulation_steps == 0:
            ...
            writer.add_scalar('1.0 Loss/train', running_loss, epoch * len(data_loader) + it)
            ...
            writer.add_scalar('1.1 FC Loss/train', fc_loss, epoch * len(data_loader) + it)
            writer.add_scalar('1.1 VQ Loss/train', vq_loss, epoch * len(data_loader) + it)
            writer.add_scalar('1.1 Perplexity', perplexity, epoch * len(data_loader) + it)
    ...
    with torch.no_grad():
        ...
        for it, res in enumerate(data_loader_val):
            ...
            loss, fc_loss, vq_loss, _ = forward_pass(model, res, vit_loss)
                
            logging.info(f"***** [epoch {epoch}] val loss: {loss.item()} *****")
            
            writer.add_scalar('1.0 Loss/val', loss.item(), epoch * len(data_loader_val) + it)
            writer.add_scalar('1.1 FC Loss/train', fc_loss, epoch * len(data_loader_val) + it)
            writer.add_scalar('1.1 VQ Loss/train', vq_loss, epoch * len(data_loader_val) + it)
...
class VQVITLoss(nn.Module):
    def __init__(self, fc_coeff=1, vq_coeff=1, label_smoothing=0.2):
        super().__init__()
        self.fc_coeff = fc_coeff
        self.vq_coeff = vq_coeff
        
        self.ce_loss = nn.CrossEntropyLoss(label_smoothing=label_smoothing).cuda()
        
    def forward(self, preds, targets, vq_loss):
        fc_loss = self.ce_loss(preds, targets)
        total_loss = self.fc_coeff*fc_loss + self.vq_coeff*vq_loss
        
        return total_loss, fc_loss, vq_loss
